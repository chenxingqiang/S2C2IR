// RUN: python3 %S/../../runtime/ascend/record_ascend.py --emit-storage-measured-from-ntile %S/../../docs/design/v3-dataset/storage-measured-ntile-4090.log | grep -v '^{' | FileCheck %s --check-prefix=EMIT
// RUN: s2c2-opt %S/storage-ntile.mlir --s2c2-evidence-bounded-schedule="profile=rtx4090 measured-cost-table=%S/../../docs/design/v3-dataset/storage-measured-v1-ntile-4090.jsonl" --check-s2c2-execution 2>&1 | grep -E 'hierarchy-global-schedule|hierarchy-global-measured|hierarchy-global-cost-coincide' | FileCheck %s --check-prefix=DEV
...
// RUN: s2c2-opt %S/storage-ntile.mlir --s2c2-evidence-bounded-schedule="profile=910B measured-cost-table=%S/../../docs/design/v3-dataset/storage-measured-v1-ntile-4090.jsonl" --check-s2c2-execution 2>&1 | grep hierarchy-global-measured | FileCheck %s --check-prefix=CROSS
// RUN: s2c2-opt %S/storage-ntile.mlir --s2c2-evidence-bounded-schedule="profile=rtx4090 measured-cost-table=%S/../../docs/design/v3-dataset/storage-measured-v1-ntile-4090.jsonl dump-schedule=%t.json" --check-s2c2-execution >/dev/null 2>&1
...
// rewrite the winner. Does not overwrite frozen 5A/5B tables.
// #69 untouched.
