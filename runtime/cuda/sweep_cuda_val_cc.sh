#!/usr/bin/env bash
# CUDA Validation C_light || C_heavy sweep.
...
BIN=${1:-./s2c2-cuda-run}
OUT=${2:-./v3-cuda-clight}
CMD=(python3 "$HERE/record_v3.py" --cuda-val-cc-sweep "$BIN" --out "$OUT")
if [[ -n "${S2C2_GIT_COMMIT:-}" ]]; then
  CMD+=(--git-commit "$S2C2_GIT_COMMIT")
fi
"${CMD[@]}"
python3 "$HERE/record_v3.py" --analyze-cuda-val-cc "${OUT}.jsonl" \
  --out "${OUT}-slices.csv"
