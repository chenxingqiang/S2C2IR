#!/usr/bin/env bash
# 910B per-candidate measured-storage-v1 from @ssd_hierarchy_lifetime.
# One timed arm per legal signature. Does not grow F. Does not
# re-measure frozen pipeline S0/S1. Does not change Cost v0.4, #69,
# or rewrite. Do not FileCheck microseconds. Do not compare 4090 μs
# to 910B μs. Do not store host/password/IP.
set -euo pipefail
HERE=$(cd "$(dirname "$0")" && pwd)
BIN=${1:-./s2c2-ascend-run}
OUT=${2:-./v3-storage-measured-hierarchy-910b}
