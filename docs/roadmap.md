...
`R` inside a declared finite family. `--s2c2-enumerate` prints that
list (`Output = R ∩ F`). Does not pick a unique `M*`, rewrite `P`, or
generate new space maps. Heuristic searcher and placement remain later.
