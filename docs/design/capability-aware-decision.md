...
Next compiler-facing increment (not this document): freeze
the hardware-neutral Capability Schema (`#59`), then a
**query** over measured cells, then a small schedule
filter. Capability data must enter the compiler decision
path or it remains a benchmark archive.
