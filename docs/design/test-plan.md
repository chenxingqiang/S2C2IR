...
| CAPA-1 | `test/Integration/capability-applicability.mlir` | `--print-capability-applicability-contract`; `Decision.subject=applicable`; applicable ≠ usable ≠ sufficient; closed cpu/cuda/rocm/sycl/ascend/npu/cim; `rewrite-license=no` |
| CAPA-2 | same | matrix: missing-evidence / unknown-target / unknown-kind=`sufficient` / usable-ne-applicable / ignore-other-device / duplicate-identity / identity-kind-mismatch / ascend-dma-present |
